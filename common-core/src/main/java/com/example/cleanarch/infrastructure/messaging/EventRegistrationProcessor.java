...
public class EventRegistrationProcessor {
    ...
    private void scanAndRegisterEventTypes(String basePackage) {
        // Scan for shared events in common package
        String commonEventsPackage = "com.example.cleanarch.common.domain.events";
        logger.info("Scanning for shared events in package: " + commonEventsPackage);
        scanEventPackage(commonEventsPackage);

        // Scan for module-specific events
        String moduleEventsPackage = basePackage + ".domain.events";
        logger.info("Scanning for module-specific events in package: " + moduleEventsPackage);
        scanEventPackage(moduleEventsPackage);
    }

    private void scanEventPackage(String packageName) {
        ...
        for (BeanDefinition beanDefinition : candidateComponents) {
            try {
                ...
                logger.info("Auto-registered event: " + eventClass.getSimpleName() + " from package: " + packageName);
            } catch (ClassNotFoundException e) {
